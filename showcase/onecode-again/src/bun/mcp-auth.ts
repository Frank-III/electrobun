  ...
  Bun.serve({
    ...
    fetch: (req) => {
      ...
      if (url.pathname === "/callback") {
        ...
        if (code && state) {
          handleMcpOAuthCallback(code, state).catch((error) => {
...
export async function startMcpOAuth(
  ...
  const serverConfig = await getMcpServerConfig(config, projectPath, serverName);
...
export async function handleMcpOAuthCallback(code: string, state: string): Promise<void> {
  ...
  try {
    ...
    const config = await readClaudeConfig();
    const serverUrl = (await getMcpServerConfig(config, pending.projectPath, pending.serverName))?.url;
...
export async function refreshMcpToken(
  ...
  try {
    const config = await readClaudeConfig();
    let serverConfig = await getMcpServerConfig(config, projectPath, serverName);
    let resolvedProjectPath = projectPath;

    // Fallback to global MCP servers if not found or missing URL in project scope.
    if (!serverConfig?.url) {
      const globalConfig = await getMcpServerConfig(config, GLOBAL_MCP_PATH, serverName);
      if (globalConfig?.url) {
...
async function saveTokensToClaudeJson(
  ...
  await updateClaudeConfigAtomic(async (config) => {
    ...
    const existingConfig = (await getMcpServerConfig(config, projectPath, serverName)) || {};
    ...
    return await updateMcpServerConfig(config, projectPath, serverName, {
      // SDK-required fields
      type: serverType,
      headers,
...
export async function getMcpAuthStatus(
  ...
  try {
    const config = await readClaudeConfig();
    const oauth = (await getMcpServerConfig(config, projectPath, serverName))?._oauth;
