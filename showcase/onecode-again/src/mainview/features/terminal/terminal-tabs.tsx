...
const TerminalTab = memo(forwardRef<HTMLButtonElement, TerminalTabProps>(function TerminalTab({ terminal, isActive, isOnly, isTruncated, cwd, initialCwd, isEditing, hasTabsToRight, canCloseOthers, small, onSelect, onClose, onCloseOthers, onCloseToRight, onRename, onEditingChange, onStartRename, textRef }, ref) {
	...
	const [editValue, setEditValue] = createSignal(terminal.name);
	...
	return <ContextMenu>
        <ContextMenuTrigger asChild>
          <button ref={ref} onClick={handleClick} onDoubleClick={handleDoubleClick} class={cn("group relative flex items-center rounded-md transition-colors h-6 flex-shrink-0 select-none", small ? "text-xs" : "text-sm", !isOnly ? "cursor-pointer" : "cursor-default", "outline-offset-2 focus-visible:outline focus-visible:outline-2 focus-visible:outline-ring/70", "overflow-hidden px-1.5 py-0.5 whitespace-nowrap min-w-[50px] gap-1.5", isActive ? "bg-muted text-foreground max-w-[180px]" : "hover:bg-muted/80 max-w-[150px]")}>
            ...
            {isEditing ? <input ref={inputRef} type="text" value={editValue} onChange={(e) => setEditValue(e.target.value)} onKeyDown={handleKeyDown} onBlur={handleBlur} onClick={(e) => e.stopPropagation()} class={cn("relative z-0 text-left flex-1 min-w-0 pr-1 bg-transparent outline-none border-none", small ? "text-xs" : "text-sm")} /> : <span ref={textRef} class="relative z-0 text-left flex-1 min-w-0 pr-1 overflow-hidden flex items-center gap-1.5 whitespace-nowrap select-none cursor-[inherit]">
                <span>{terminal.name}</span>
                {shortPath && <span class="text-muted-foreground">{shortPath}</span>}
              </span>}

            { /* Gradient fade on the right when text is truncated */}
            {isTruncated && !isEditing && <div class={cn("absolute right-0 top-0 bottom-0 w-6 pointer-events-none z-[1] rounded-r-md opacity-100 group-hover:opacity-0 transition-opacity duration-200", isActive ? "bg-gradient-to-l from-muted to-transparent" : "bg-gradient-to-l from-background to-transparent")} />}
...
export const TerminalTabs = memo(function TerminalTabs({ terminals, activeTerminalId, cwds, initialCwd, terminalBg, hidePlusButton = false, small = false, onSelectTerminal, onCloseTerminal, onCloseOtherTerminals, onCloseTerminalsToRight, onCreateTerminal, onRenameTerminal }: TerminalTabsProps) {
	...
	const [truncatedTabs, setTruncatedTabs] = createSignal(new Set());
	...
	const [editingTerminalId, setEditingTerminalId] = createSignal(null);
	...
	return <div class="relative flex-1 min-w-0 flex items-center h-7">
      {	/* Left gradient */}
      {showLeftGradient && <div class="absolute left-0 top-0 bottom-0 w-8 pointer-events-none z-30" style={{ background: terminalBg ? `linear-gradient(to right, ${terminalBg}, transparent)` : undefined }} />}

      { /* Scrollable tabs container - with padding-right for plus button */}
      <div ref={tabsContainerRef} class={cn("flex items-center px-1 py-1 -my-1 gap-1 flex-1 min-w-0 overflow-x-auto scrollbar-hide", !hidePlusButton && "pr-12")} style={{ WebkitAppRegion: "no-drag" }}>
        {terminals.map((terminal, index) => {
 const hasTabsToRight = index < terminals.length - 1;
		const canCloseOthers = terminals.length > 1;
		return <TerminalTab key={terminal.id} ref={(el) => {
			if (el) {
				tabRefs.current.set(terminal.id, el);
			} else {
				tabRefs.current.delete(terminal.id);
			}
		}} terminal={terminal} isActive={terminal.id === activeTerminalId} isOnly={isOnly} isTruncated={truncatedTabs.has(terminal.id)} cwd={cwds[terminal.paneId]} initialCwd={initialCwd} isEditing={editingTerminalId === terminal.id} hasTabsToRight={hasTabsToRight} canCloseOthers={canCloseOthers} small={small} onSelect={onSelectTerminal} onClose={onCloseTerminal} onCloseOthers={() => onCloseOtherTerminals(terminal.id)} onCloseToRight={() => onCloseTerminalsToRight(terminal.id)} onRename={onRenameTerminal} onEditingChange={(isEditing) => handleEditingChange(terminal.id, isEditing)} onStartRename={() => handleStartRename(terminal.id)} textRef={(el) => {
			if (el) {
      ...
      {!hidePlusButton && <div class="absolute right-0 top-0 bottom-0 flex items-center z-20" style={{ WebkitAppRegion: "no-drag" }}>
          ...
          <div class="w-6 h-full" style={{ background: terminalBg ? `linear-gradient(to right, transparent, ${terminalBg})` : undefined }} />
          <div class="h-full flex items-center pr-1" style={{ backgroundColor: terminalBg }}>
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="ghost" size="icon" onClick={onCreateTerminal} class="h-6 w-6 p-0 hover:bg-foreground/10 transition-[background-color,transform] duration-150 ease-out active:scale-[0.97] rounded-md" aria-label="New terminal">
