        ...
        <ContextMenuTrigger asChild>
          ...
          >
            ...
            {isEditing ? (
              ...
              <span
                ref={textRef}
                class="relative z-0 text-left flex-1 min-w-0 pr-1 overflow-hidden flex items-center gap-1.5 whitespace-nowrap select-none cursor-[inherit]"
              >
                <span>{terminal.name}</span>
                {shortPath && (
                  <span class="text-muted-foreground">{shortPath}</span>
                )}
              </span>
            )}
            ...
            {!isOnly && !isEditing && (
              <div class="absolute right-0 top-0 bottom-0 flex items-center justify-end pr-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200 z-10">
                ...
                <button
                  ...
                  class="relative z-20 hover:text-foreground rounded p-0.5 transition-[color,transform] duration-150 ease-out active:scale-[0.97]"
...
export const TerminalTabs = memo(function TerminalTabs({
  ...
  return (
    <div class="relative flex-1 min-w-0 flex items-center h-7">
      {/* Left gradient */}
      {showLeftGradient && (
        <div
          class="absolute left-0 top-0 bottom-0 w-8 pointer-events-none z-30"
          style={{
      ...
      {!hidePlusButton && (
        <div
          class="absolute right-0 top-0 bottom-0 flex items-center z-20"
          style={{
            // @ts-expect-error - WebKit-specific property for Electron
            WebkitAppRegion: "no-drag",
          }}
        >
          ...
          />
          <div
            class="h-full flex items-center pr-1"
