          ...
          <TooltipTrigger asChild>
            <PopoverTrigger asChild>
              <Button
                variant="ghost"
                size="icon"
                class="h-6 w-6 p-0 hover:bg-foreground/10 transition-[background-color,transform] duration-150 ease-out active:scale-[0.97] flex-shrink-0 rounded-md"
...
export function AgentsSubChatsSidebar({
  ...
  return (
    <div
      class="flex flex-col h-full bg-background border-r overflow-hidden relative"
    ...
    >
            ...
            <Tooltip delayDuration={500}>
              <TooltipTrigger asChild>
                <Button
                  ...
                  variant="outline"
                  size="sm"
                  class="h-7 px-2 w-full hover:bg-foreground/10 transition-[background-color,transform] duration-150 ease-out active:scale-[0.97] text-foreground rounded-lg"
                >
      ...
      >
        ...
        {isLoading ? (
          ...
          <>
            {/* Top gradient */}
            {showTopGradient && (
              <div class="absolute left-0 right-0 top-0 h-8 bg-gradient-to-b from-background to-transparent pointer-events-none z-10" />
            )}

            {/* Bottom gradient */}
            {showBottomGradient && (
              <div class="absolute left-0 right-0 bottom-0 h-8 bg-gradient-to-t from-background to-transparent pointer-events-none z-10" />
            )}
            ...
            >
              {filteredSubChats.length > 0 ? (
                <div
                  class={cn("mb-4", isMultiSelectMode ? "px-0" : "-mx-1")}
                >
                  {/* Pinned section */}
      ...
      <AnimatePresence mode="wait">
        {isMultiSelectMode && (
          <motion.div
            ...
            class="flex-shrink-0 p-2 bg-background space-y-2 relative z-10"
            style={{
              // @ts-expect-error - WebKit-specific property
              WebkitAppRegion: "no-drag",
            }}
      ...
      {typeof document !== "undefined" &&
        createPortal(
          <div
            ref={tooltipRef}
            class="fixed z-[100000] max-w-xs px-2 py-1 text-xs bg-popover border border-border rounded-md shadow-lg pointer-events-none text-foreground/90 whitespace-nowrap"
            style={{
              display: "none",
