...
import { ChevronDown, MoreHorizontal, Columns3 } from "lucide-solid";
...
import { IconDoubleChevronLeft, SettingsIcon, PlusIcon, ProfileIcon, PublisherStudioIcon, SearchIcon, GitHubLogo, LoadingDot, ArchiveIcon, TrashIcon, QuestionCircleIcon, QuestionIcon, KeyboardIcon, TicketIcon, CloudIcon } from "../../components/ui/icons";
...
export function AgentsSidebar(props: AgentsSidebarProps) {
	...
	const handleBulkArchive = () => {
		...
		const handlePartialSuccess = (archivedIds: string[], isRemote: boolean) => {
			...
			if (isRemote) {
				const newItems: UndoItem[] = archivedIds.map((id) => {
					const timeoutId = setTimeout(() => removeWorkspaceFromStack(`remote_${id}`), 1e4);
					return {
						type: "workspace" as const,
						chatId: `remote_${id}`,
						timeoutId,
		...
		if (remoteIds.length > 0) {
			archiveRemoteChatsBatchMutation.mutate(remoteIds, { onSuccess: () => handlePartialSuccess(remoteIds, true) });
		}
		// Archive local chats
		if (localIds.length > 0) {
			archiveChatsBatchMutation.mutate({ chatIds: localIds }, { onSuccess: () => handlePartialSuccess(localIds, false) });
		}
	};
	const handleArchiveAllBelow = (chatId: string) => {
		...
		// Archive remote chats
		if (remoteIds.length > 0) {
			archiveRemoteChatsBatchMutation.mutate(remoteIds, { onSuccess: () => {
