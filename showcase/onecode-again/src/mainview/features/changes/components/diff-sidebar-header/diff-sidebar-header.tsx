...
export const DiffSidebarHeader = memo(function DiffSidebarHeader({
	...
	return (
		<div class="relative flex items-center justify-between h-10 px-2 border-b border-border/50 bg-background flex-shrink-0">
			...
			<div
				class="relative z-10 flex items-center gap-1 min-w-0 flex-shrink"
				style={{
					// @ts-expect-error - WebKit-specific property
					...
					<ContextMenu>
						<ContextMenuTrigger asChild>
							<a
								...
								class="inline-flex items-center gap-1 h-6 px-2 rounded-md hover:bg-foreground/10 transition-colors"
							>
			...
			<div
				class="relative z-10 flex items-center gap-1 flex-shrink-0"
				style={{
					// @ts-expect-error - WebKit-specific property
					WebkitAppRegion: "no-drag",
				}}
					...
					<Tooltip>
						<TooltipTrigger asChild>
							<Button
								...
								class="h-6 px-2 gap-1 text-xs hover:bg-foreground/10"
							>
								{isReviewing ? (
									<IconSpinner class="size-3.5" />
								) : (
									<IconReview class="size-3.5" />
								)}
								<span>Review</span>
							</Button>
						</TooltipTrigger>
						...
						<DropdownMenu>
							...
							<DropdownMenuContent align="end" class="w-52">
								...
								<DropdownMenuItem
									...
									class="text-xs"
								>
									<HiArrowPath class={cn("mr-2 size-3.5", isFetchPending && "animate-spin")} />
									<div class="flex-1">
								...
								{hasUpstream && pullCount > 0 && (
									<DropdownMenuItem
										...
										class="text-xs data-[highlighted]:bg-red-500/15 data-[highlighted]:text-red-400 [&_div]:data-[highlighted]:text-red-400/70"
									>
							...
							<>
								<DropdownMenuSub>
									...
									<DropdownMenuSubContent>
										<DropdownMenuItem
											onClick={() => onViewModeChange(DiffModeEnum.Split)}
											class={cn("text-xs", viewMode === DiffModeEnum.Split && "bg-muted")}
										>
										...
										<DropdownMenuItem
											onClick={() => onViewModeChange(DiffModeEnum.Unified)}
											class={cn("text-xs", viewMode === DiffModeEnum.Unified && "bg-muted")}
										>
