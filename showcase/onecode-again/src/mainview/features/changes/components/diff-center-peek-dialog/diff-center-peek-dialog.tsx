...
export function DiffCenterPeekDialog({ isOpen, onClose, children }: DiffCenterPeekDialogProps) {
	...
	return <AnimatePresence>
      {isOpen && <>
          {	/* Backdrop */}
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: .15 }} class="fixed inset-0 bg-black/50 z-50" onClick={onClose} />

          { /* Dialog */}
          <motion.div role="dialog" aria-modal="true" initial={{
 opacity: 0,
		scale: .98
	}} animate={{
		opacity: 1,
