...
export function ChangesFileFilter({
	...
	return (
		<div class={cn("flex flex-col gap-1.5 px-2 py-1.5", className)}>
			{/* Search row */}
			<div class="flex items-center gap-1">
				{/* Search input */}
				<div class="relative flex-1">
					<LuSearch class="absolute left-2.5 top-1/2 -translate-y-1/2 size-3.5 text-muted-foreground pointer-events-none" />
					<Input
						type="search"
						value={value}
						onChange={(e) => onChange(e.target.value)}
						placeholder={placeholder}
						class="h-7 pl-7 pr-7 text-xs bg-muted/50"
					/>
					{value && (
						<button
							type="button"
							onClick={() => onChange("")}
							class="absolute right-2 top-1/2 -translate-y-1/2 p-0.5 rounded hover:bg-muted-foreground/20 transition-colors"
						>
							<LuX class="size-3 text-muted-foreground" />
						</button>
					)}
				</div>
			...
			{selectedSubChat && (
				<div class="flex items-center justify-between gap-2 h-7 px-2 rounded-md bg-muted/80 border border-border/50">
					...
					<button
						...
						class="p-0.5 rounded hover:bg-foreground/10 transition-colors flex-shrink-0"
					>
						<LuX class="w-3.5 h-3.5 text-muted-foreground" />
					</button>
				</div>
			)}
		</div>
