...
import { WIDGET_REGISTRY, widgetVisibilityAtomFamily, widgetOrderAtomFamily, type WidgetId } from "./atoms";
...
export function WidgetSettingsPopup({ workspaceId, isRemoteChat = false }: WidgetSettingsPopupProps) {
	...
	return <Popover>
      <PopoverTrigger asChild>
        <Button variant="ghost" size="sm" class="h-6 px-2 text-xs text-muted-foreground hover:text-foreground hover:bg-foreground/10 transition-colors rounded-md">
          Edit widgets
        </Button>
      </PopoverTrigger>
