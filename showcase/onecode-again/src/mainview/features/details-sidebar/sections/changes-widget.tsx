...
*/
export function ChangesWidget(props: ChangesWidgetProps) {
	const chatId = () => props.chatId;
	const worktreePath = () => props.worktreePath;
	const isCommitting = () => props.isCommitting ?? false;
	const diffDisplayMode = () => props.diffDisplayMode ?? "side-peek";
	...
	const displayFiles = () => props.parsedFileDiffs ?? [];
	const displayStats = () => props.diffStats;
	...
	<Show when={openDiffHotkey}><Kbd>{openDiffHotkey}</Kbd></Show>
		...
		{ /* Content */}
		<Show when={hasChanges()} fallback={<div class="text-xs text-muted-foreground px-2 py-2">
				No changes
			</div>}>
	...
	{ /* View diff button */}
			<Button variant="outline" size="sm" class={cn("h-7 text-xs", props.onCommit ? "flex-1" : "w-full")} onClick={() => props.onExpand?.()}>
				View Diff
			</Button>
		</div>
		</Show>
