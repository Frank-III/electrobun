...
export function QueueProcessor() {
	...
	// Function to process queue for a specific sub-chat (defined outside effect to avoid recreation)
	...
	// Uses untrack to avoid infinite loops when modifying status inside effect
	const checkAllQueues = () => {
		...
		for (const subChatId of Object.keys(queues)) {
			...
			if ((status === "ready" || status === "error") && !processing.has(subChatId)) {
				// If error status, clear it before retrying (done outside reactive tracking)
				if (status === "error") {
					// Schedule the status change to happen after this effect completes
					queueMicrotask(() => {
