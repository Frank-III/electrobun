  ...
  const getIcon = () => {
    ...
    if (isVoiceMode) {
      ...
      if (isRecording) {
        ...
        return (
          <div class="w-2.5 h-2.5 bg-current rounded-[2px] flex-shrink-0 mx-auto" />
        )
      }
    ...
    if (isVoiceMode) {
      ...
      return (
        <div class="flex flex-col items-start gap-0.5">
          ...
          {voiceHotkey && (
            <span class="text-muted-foreground">{voiceHotkey}</span>
          )}
        </div>
      )
    }
  ...
  return (
    <Tooltip delayDuration={1_000} open={tooltipOpen}>
      <TooltipTrigger asChild>
        <Button
          size={size}
          class={`h-7 w-7 rounded-full transition-[background-color,transform,opacity] duration-150 ease-out active:scale-[0.97] flex items-center justify-center ${glowClass || ""} ${modeClass} ${className}`}
