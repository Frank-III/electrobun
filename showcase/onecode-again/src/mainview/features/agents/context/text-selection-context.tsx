	...
	createEffect(() => {
		...
		const handleSelectionChange = () => {
			...
			rafId = requestAnimationFrame(() => {
				...
				const messageElement = element?.closest?.("[data-assistant-message-id]") as HTMLElement | null;
				// Check for tool-edit (Edit/Write tool in chat)
				// Use specific selector for Edit/Write tools only
				const toolEditElement = element?.closest?.("[data-part-type=\"tool-Edit\"], [data-part-type=\"tool-Write\"]") as HTMLElement | null;
				...
				const diffElement = diffWrapperElement?.closest?.("[data-diff-file-path]") as HTMLElement | null;
