...
export function ChatSearchBar({ messages, className, topOffset }: ChatSearchBarProps) {
  ...
  return (
    ...
    >
      ...
      <div class="w-[128px] flex items-center justify-end shrink-0">
        {countInfo.total > 0 ? (
          <>
            ...
            <button
              type="button"
              class="h-6 w-6 flex items-center justify-center rounded-md cursor-pointer text-muted-foreground hover:text-foreground hover:bg-muted active:scale-95 transition-all duration-150 ease-out"
              onClick={() => {
                goToPrev()
                inputRef.current?.focus()
              }}
      ...
      <button
        type="button"
        class="h-6 w-6 shrink-0 flex items-center justify-center rounded-md cursor-pointer text-muted-foreground hover:text-foreground hover:bg-muted active:scale-95 transition-all duration-150 ease-out"
        onClick={() => closeSearch()}
        title="Close (Esc)"
      >
        <X class="h-4 w-4" />
