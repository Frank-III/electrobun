...
*/
export function VoiceWaveIndicator({ isRecording, audioLevel, className }: VoiceWaveIndicatorProps) {
	const [barsRef, setBarsRef] = createSignal<HTMLDivElement[]>([]);
	createEffect(() => {
		if (!isRecording) {
	...
	return <div class={cn("flex items-center justify-center gap-[3px] h-5 px-2", className)}>
      {[
		0,
		1,
		2,
