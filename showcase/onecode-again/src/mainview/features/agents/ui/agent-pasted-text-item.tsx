...
export function AgentPastedTextItem(props: AgentPastedTextItemProps) {
	const [local] = splitProps(props, ["filePath", "filename", "size", "preview", "onRemove"]);
	const [isHovered, setIsHovered] = createSignal(false);
	// Get a short title from the preview
	const title = local.preview.split("\n")[0]?.slice(0, 20) || local.preview.slice(0, 20);
	const displayTitle = title.length < local.preview.length ? `${title}...` : title;
	return <HoverCard openDelay={300} closeDelay={100}>
      <HoverCardTrigger asChild>
        <div class="relative flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-muted/50 cursor-default min-w-[120px] max-w-[200px]" onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
          {	/* Icon container */}
          ...
          <div class="flex flex-col min-w-0">
            <span class="text-sm font-medium text-foreground truncate">
	          ...
	          <span class="text-xs text-muted-foreground">
	            Pasted Text · {formatSize(local.size)}
	          </span>
	        </div>
	      ...
	      <HoverCardContent side="top" align="start" class="w-80">
	        <div class="flex flex-col gap-1">
	          <div class="flex items-center gap-1.5 text-xs text-muted-foreground">
	            ...
	            <span>Pasted text · {formatSize(local.size)}</span>
	          </div>
	          <pre class="text-sm whitespace-pre-wrap break-words font-mono max-h-32 overflow-y-auto">
	            {local.preview}
	          </pre>
	        </div>
	      </HoverCardContent>
