...
export function QuickCommentInput({ selectedText, source, rect, onSubmit, onCancel }: QuickCommentInputProps) {
	...
	// Get source label
	const sourceLabel = source.type === "diff" || source.type === "tool-edit" ? `${source.filePath.split("/").pop()}${source.type === "diff" && source.lineNumber ? `:${source.lineNumber}` : ""}` : "from chat";
	// Animation: scale from direction of selection
	const animationClass = showBelow ? "animate-in fade-in-0 zoom-in-95 origin-top duration-100" : "animate-in fade-in-0 zoom-in-95 origin-bottom duration-100";
	const content = <div ref={containerRef} style={style} class={animationClass}>
      <div class="rounded-md bg-popover border border-border shadow-lg overflow-hidden">
        ...
        <div class="p-1.5">
          <div class="flex items-center gap-1.5">
            <input ref={inputRef} type="text" value={comment} onChange={(e) => setComment(e.target.value)} onKeyDown={handleKeyDown} placeholder="Add your reply..." class="flex-1 text-xs bg-transparent outline-none text-foreground placeholder:text-muted-foreground px-1" />
