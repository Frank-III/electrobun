...
function CodeSelectIcon(props: {
	...
	return <svg viewBox="0 0 24 24" fill="none" class={local.class}>
...
export function AgentDiffTextContextItem(props: AgentDiffTextContextItemProps) {
	const [local] = splitProps(props, ["text", "preview", "filePath", "lineNumber", "lineType", "onRemove"]);
	const [isHovered, setIsHovered] = createSignal(false);
	// Check if this is a plan selection
	const isPlan = isPlanFile(local.filePath);
	// Extract just the filename from the path
	const fileName = local.filePath.split("/").pop() || local.filePath;
	return <div class="relative flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-muted/50 cursor-default min-w-[120px] max-w-[200px]" onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
      {	/* Icon container */}
      <div class="flex items-center justify-center size-8 rounded-md bg-muted shrink-0">
        <Show when={isPlan} fallback={<CodeSelectIcon class="size-4 text-muted-foreground" />}>
      ...
      <div class="flex flex-col min-w-0">
        <span class="text-sm font-medium text-foreground truncate">
          <Show when={isPlan} fallback={fileName}>
            Plan
          </Show>
        </span>
        <span class="text-xs text-muted-foreground flex items-center gap-1">
	          <Show when={isPlan} fallback={<>
	            <Show when={local.lineNumber}><span>Line {local.lineNumber}</span></Show>
	            <Show when={local.lineType}><span class={local.lineType === "new" ? "text-green-500" : "text-red-500"}>
	                {local.lineNumber ? "· " : ""}{local.lineType === "new" ? "Added" : "Removed"}
	              </span></Show>
	            {!local.lineNumber && !local.lineType && "Code selection"}
	          </>}>
            Text selection
          </Show>
        </span>
