...
export const McpServersIndicator = memo(function McpServersIndicator({ projectPath }: McpServersIndicatorProps) {
	...
	const [expandedServers, setExpandedServers] = createSignal(new Set());
	...
	const getStatusIcon = (status: MCPServerStatus) => {
		switch (status) {
			...
			case "pending": return <Loader2 class="w-3 h-3 text-muted-foreground animate-spin" aria-label="Connecting" />;
	...
	return <Popover open={isOpen} onOpenChange={setIsOpen}>
      ...
      <PopoverContent align="start" class="w-72 p-0" onOpenAutoFocus={(e) => e.preventDefault()} onKeyDown={handleKeyDown} role="dialog" aria-label="MCP Servers">
