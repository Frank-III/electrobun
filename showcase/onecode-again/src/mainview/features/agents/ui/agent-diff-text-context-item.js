...
exports.AgentDiffTextContextItem = AgentDiffTextContextItem;
...
function AgentDiffTextContextItem(_a) {
    var text = _a.text, preview = _a.preview, filePath = _a.filePath, lineNumber = _a.lineNumber, lineType = _a.lineType, onRemove = _a.onRemove;
    var _b = (0, solid_js_1.createSignal)(false), isHovered = _b[0], setIsHovered = _b[1];
    // Check if this is a plan selection
    var isPlan = (0, agent_tool_utils_1.isPlanFile)(filePath);
    ...
    return <div class="relative flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-muted/50 cursor-default min-w-[120px] max-w-[200px]" onMouseEnter={function () { return setIsHovered(true); }} onMouseLeave={function () { return setIsHovered(false); }}>
      ...
      <div class="flex items-center justify-center size-8 rounded-md bg-muted shrink-0">
        {isPlan ? <TextSelectIcon class="size-4 text-muted-foreground"/> : <CodeSelectIcon class="size-4 text-muted-foreground"/>}
      </div>
