...
export function AgentPreview({ chatId, sandboxId, port, repository, hideHeader = false, onClose, isMobile = false }: AgentPreviewProps) {
	...
	const [iframeRef, setIframeRef] = createSignal<HTMLIFrameElement>(null);
	const [frameRef, setFrameRef] = createSignal<HTMLDivElement>(null);
 ...
 width: `${100 / scale * 100}%`,
		...
		transformOrigin: "top left"
              ...
              <iframe ref={iframeRef} key={reloadKey} src={previewUrl} width="100%" height="100%" style={{ border: "none" }} title="Preview" sandbox="allow-scripts allow-same-origin allow-forms allow-popups allow-modals" allow="clipboard-write" onLoad={() => setIsLoaded(true)} onError={() => setIsLoaded(true)} />
            </div>
            {	/* Loading overlay */}
            ...
            <div ref={frameRef} class={cn("relative overflow-hidden flex-shrink-0 bg-background", !isResizing && "transition-[width,height,margin] duration-300 ease-in-out", viewportMode === "desktop" ? "border-[0.5px] rounded-sm" : "shadow-lg border")} style={{
