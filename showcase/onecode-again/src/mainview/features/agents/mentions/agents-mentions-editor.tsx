...
const LARGE_TEXT_THRESHOLD = 1e4;
...
type AgentsMentionsEditorProps = {
	...
	onCloseTrigger: () => void;
	onSlashTrigger?: (payload: TriggerPayload) => void;
...
function buildContentFromSerialized(root: HTMLElement, serialized: string, resolveMention?: (id: string) => FileMentionOption | null) {
	// Clear safely
	...
	while ((match = regex.exec(serialized)) !== null) {
		...
		if (!option && id.startsWith(MENTION_PREFIXES.TOOL)) {
			...
			const displayName = toolName.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase()).trim();
			option = {
				id,
				label: displayName,
				path: toolPath,
			...
			if (cursorInThisNode) {
				...
				for (let i = 0; i < textBeforeInNode.length; i++) {
					if (textBeforeInNode[i] === "/") {
						...
						const charBefore = globalSlashIdx === 0 ? null : (serialized + textBeforeInNode.slice(0, i)).charAt(globalSlashIdx - 1);
