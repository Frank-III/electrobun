...
export function getFileIconByExtension(filename: string, returnNullForUnknown = false) {
	...
	switch (ext) {
		...
		case "js":
		case "mjs":
		case "cjs": return JavaScriptIcon;
		case "jsx": return ReactIcon;
		case "py":
		case "pyw":
		case "pyi": return PythonIcon;
		...
		case "html":
		case "htm": return HTMLIcon;
		case "scss":
		case "sass": return SCSSIcon;
		case "json":
		case "jsonc": return JSONIcon;
		case "yaml":
		case "yml": return YAMLIcon;
		case "sh":
		case "bash":
		case "zsh": return ShellIcon;
		case "sql": return SQLIcon;
		case "graphql":
		case "gql": return GraphQLIcon;
		case "prisma": return PrismaIcon;
		case "dockerfile": return DockerIcon;
		case "toml": return TOMLIcon;
		case "env":
 // This handles .env files, but we already handled them above
		...
		case "cc":
		case "cxx":
		case "hpp": return CppIcon;
		case "cs": return CSharpIcon;
		case "php": return PHPIcon;
		case "rb": return RubyIcon;
		case "kt": return KotlinIcon;
		case "vue": return VueIcon;
		case "svelte": return SvelteIcon;
		case "astro": return AstroIcon;
		case "swift": return SwiftIcon;
		case "pdf": return PDFIcon;
		case "svg": return SVGIcon;
		default: return returnNullForUnknown ? null : FilesIcon;
	}
}
...
function formatToolName(toolName: string): string {
	return toolName.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase()).replace(/\s+/g, " ").trim();
}
...
export function createFileIconElement(filename: string, type?: "file" | "folder" | "skill" | "agent" | "category" | "tool"): SVGSVGElement {
	const IconComponent = type === "skill" ? SkillIcon : type === "agent" ? CustomAgentIcon : type === "tool" ? ToolIcon : type === "folder" ? FolderOpenIcon : getFileIconByExtension(filename) ?? FilesIcon;
...
*/
function sortFilesByRelevance<T extends {
	label: string;
	path?: string;
