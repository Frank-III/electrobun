...
function TextSelectIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="none" class={className}>
...
function MentionChip({ mention }: { mention: ParsedMention }) {
  ...
  return (
    <span
      class="inline-flex items-center gap-1 px-[6px] rounded-[6px] text-sm align-middle bg-black/[0.04] dark:bg-white/[0.08] text-foreground/80 select-none"
      title={title}
    >
...
function renderTextWithUltrathink(text: string): React.ReactNode {
  ...
  return parts.map((part, index) => {
    if (part.toLowerCase() === "ultrathink") {
      return (
        <span key={index} class="chroma-text chroma-text-animate">
          {part}
        </span>
      )
    }
  ...
  return (
    <HoverCard openDelay={300} closeDelay={100}>
      ...
      <HoverCardContent
        side="top"
        align="start"
        class="w-80 max-h-48 overflow-y-auto"
      >
