...
export const AgentsSlashCommand = memo(function AgentsSlashCommand({ isOpen, onClose, onSelect, searchText, position, projectPath, mode, disabledCommands }: AgentsSlashCommandProps) {
	...
	const requestedHeight = Math.min(options.length * itemHeight + headersCount * headerHeight + 8, 200);
	const gap = 8;
	...
	return createPortal(<div ref={dropdownRef} class="fixed z-[99999] overflow-y-auto rounded-[10px] border border-border bg-popover py-1 text-xs text-popover-foreground shadow-lg dark [&::-webkit-scrollbar]:hidden" style={{
		top: finalTop,
		left: finalLeft,
 ...
 const isSelected = selectedIndex === index;
		return <div key={option.id} data-option-index={index} onMouseDown={(e) => {
			e.preventDefault();
			e.stopPropagation();
			handleSelect(option);
		}} onMouseEnter={() => setSelectedIndex(index)} class={cn("group inline-flex w-[calc(100%-8px)] mx-1 items-center whitespace-nowrap outline-none", "h-7 px-1.5 justify-start text-xs rounded-md", "transition-colors cursor-pointer select-none", isSelected ? "dark:bg-neutral-800 bg-accent text-foreground" : "text-muted-foreground dark:hover:bg-neutral-800 hover:bg-accent hover:text-foreground")}>
