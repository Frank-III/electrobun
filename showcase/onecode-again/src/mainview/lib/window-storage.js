...
exports.createWindowScopedStorage = createWindowScopedStorage;
exports.atomWithWindowStorage = atomWithWindowStorage;
exports.createWindowStoredSignal = createWindowStoredSignal;
var jotai_utils_1 = require("./state/jotai-utils");
 ...
 */
function migrateFromNumericWindowId(key, targetWindowKey) {
    // Only migrate for "main" window (the primary window)
    if (!targetWindowKey.startsWith("main:"))
        return null;
    // Skip if already checked this key
    if (migratedKeys.has(targetWindowKey))
        return null;
    ...
    for (var i = 0; i < localStorage.length; i++) {
        ...
        if (match && match[2] === key) {
            ...
            if (value !== null) {
                console.log("[WindowStorage] Migrated from numeric ID: ".concat(storageKey, " to ").concat(targetWindowKey));
 ...
 */
function createWindowScopedStorage() {
    return (0, jotai_utils_1.createJSONStorage)(function () { return ({
        getItem: function (key) {
            var windowKey = "".concat((0, WindowContext_1.getWindowId)(), ":").concat(key);
            var value = localStorage.getItem(windowKey);
            ...
            if (value === null && !migratedKeys.has(windowKey)) {
                ...
                var migratedValue = migrateFromNumericWindowId(key, windowKey);
                if (migratedValue !== null) {
                    try {
                        localStorage.setItem(windowKey, migratedValue);
                    }
                    catch (e) {
                        console.warn("[WindowStorage] Failed to save migrated value for ".concat(windowKey, ":"), e);
                    }
                    value = migratedValue;
                }
                ...
                if (value === null) {
                    ...
                    if (legacyValue !== null) {
                        try {
                            ...
                            console.log("[WindowStorage] Migrated ".concat(key, " to ").concat(windowKey));
                        }
                        catch (e) {
                            console.warn("[WindowStorage] Failed to save migrated value for ".concat(windowKey, ":"), e);
                        }
        ...
        setItem: function (key, value) {
            ...
            try {
                ...
                console.error("[WindowStorage] Failed to save ".concat(windowKey, ":"), e);
            }
        },
...
function createWindowScopedRawStorage() {
    return {
        getItem: function (key) {
            var windowKey = "".concat((0, WindowContext_1.getWindowId)(), ":").concat(key);
            var value = localStorage.getItem(windowKey);
 ...
 * )
 */
function atomWithWindowStorage(key, initialValue, _options) {
    return createWindowStoredSignal(key, initialValue);
}
function createWindowStoredSignal(key, initialValue) {
    return (0, signal_storage_1.createStoredSignal)(key, initialValue, createWindowScopedRawStorage());
}
