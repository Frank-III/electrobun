...
function GitHubAvatarIcon({ gitOwner, className }: { gitOwner: string; className?: string }) {
  ...
  if (hasError) {
    return <FolderOpen class={cn("text-muted-foreground flex-shrink-0", className)} />
  }

  return (
    <div class={cn("relative flex-shrink-0", className)}>
      ...
      <img
        ...
        alt={gitOwner}
        class={cn("rounded-sm flex-shrink-0", className, isLoaded ? 'opacity-100' : 'opacity-0')}
        onLoad={handleLoad}
        onError={handleError}
      />
    </div>
    ...
    return projects.map((project) => ({
      ...
      icon: (project.gitOwner && project.gitProvider === 'github')
        ? ({ className }: { className?: string }) => (
            <GitHubAvatarIcon gitOwner={project.gitOwner!} class={className} />
          )
        : FolderOpen,
      ...
      <>
        ...
        >
          ...
          <div class="flex-1 overflow-y-auto">
            {showContent ? (
              <div class="bg-tl-background min-h-full">
                {renderTabContent()}
              </div>
            ) : (
              <div class="p-4">
        ...
        <>
          ...
          <div class="fixed top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] z-[45]">
            <motion.div
              ...
              transition={{ duration: 0.2 }}
              class="w-[90vw] h-[80vh] max-w-[900px] p-0 flex flex-col rounded-[20px] bg-background border-none bg-clip-padding shadow-2xl overflow-hidden select-none"
              role="dialog"
              aria-modal="true"
