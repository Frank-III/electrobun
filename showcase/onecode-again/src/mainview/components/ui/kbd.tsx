...
function renderShortcut(children: JSX.Element): JSX.Element {
  ...
  const symbolMap: Record<string, () => JSX.Element> = {
    ...
    "⌃": () => <span>⌃</span>, // Control stays as unicode (no icon)
  ...
  return (
    <For each={tokens}>
      {(token) => (
        <Show when={symbolMap[token]} fallback={<span>{token}</span>}>
          {symbolMap[token]?.()}
        </Show>
      )}
    </For>
...
const Kbd: ParentComponent<KbdProps> = (props) => {
  ...
  return (
    <kbd
      ref={local.ref as HTMLElement | ((el: HTMLElement) => void) | undefined}
      class={cn(
